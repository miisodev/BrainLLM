...
| `recall(query, …)` | BrainLLM-wide ranked search. `orderBy` / `orderDirection` for temporal ordering; `fastSearch` for title/label-only (faster); `regex=` for a body pattern instead of keywords; `fuzzy=` to force the typo-tolerant pass on or off (automatic when exact results are thin, and always marked `fuzzy: true`). |

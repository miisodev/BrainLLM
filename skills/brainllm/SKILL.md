...
SESSION END     session()               ← mandatory pre-close; fetches singletons + diary + sweep; follow next[]
                ...
                diary(body)              ← the day's closing record, written with the cues in hand — gate counts it only after remarks()
                close(summary, title?)  ← refuses until all steps ran AND session → remarks → diary held; force=true to override
...
ANYTIME         brain()                  ← surface the full content tree (all areas, sub-containers)
                inspect(noteId, content?) ← every label, relation + attachment on one note; content=true adds the raw body
```
...
**One-per-day notes:** diary, session, and log notes are each one per day, titled `[yyyy-mm-dd]`. `start()` creates today's diary and session stubs. These are chronological records — **every** diary and session write lands as a timestamped `Addendum — HH:mm` block, including the first of the day, so each entry reads the same. Log notes are generated by `close()` and append an addendum if close is called again the same day.
...
- The **sources gate** is mandatory: every claim traces to a Sources-note entry — ❇️ discovered/credible, ✅ used (dated only when actually read and used, never on discovery). An unsourced domain note corrupts the brain; if every source candidate is rejected, no note is created.
...
- Every information note opens with a maintained `Last updated: yyyy-mm-dd` line — the one manual date you write.
...
| `close(summary, title?, learned?, date?, backup?, force?)` | Commit the session log ([yyyy-mm-dd] note, title param above Summary) + backup + daily log. **Refuses unless `session()`, `addendum()`, `maintain()`, `remarks()`, and `diary()` each ran this session AND `session() → remarks() → diary()` held (last calls)** — returns `{error, detail, hint}` naming what's missing or out of order; `force=true` bypasses (reported back as `bypassed`). On success returns `{action, noteId, date, backup, log}` and resets the gate. Once, last. |
...
| `remember(kind, …)` | Write a note — routed, formatted, deduped server-side. `connect=[{relation, toNoteId}]` wires relations in the same call; a new connectable note without them returns an orphan-prevention hint. `icon=` sets the display icon (class or bare name, normalized). Rejects diary/session/log/domain — each has a dedicated path. |
| `diary(body, icon?, date?)` | Write to today's [yyyy-mm-dd] diary — every write lands as a timestamped `Addendum — HH:mm` block, including the first of the day. The FINAL gate step — close() counts it only when its last call came after session() and remarks(); mid-session writes welcome. |
...
| `revise(noteId, …)` | Append / replace / section-edit / find-replace a note. Section: h2/h3/h4 tolerant match, returns `matched`/`headingCount` — check them. `find=` + `body=`: exact-string surgery, returns `replaced` count. Snapshot taken on content writes (not metadata-only). |
...
| `inspect(noteId, content?)` | Full raw read of one note: every label (not just noteType/status), every outbound relation, the attachment inventory (id/title/mime/role/size), type/mime/parent/child ids, dates — and the raw body when content=true. The deep-dive counterpart to explore() and the surface reads. |
